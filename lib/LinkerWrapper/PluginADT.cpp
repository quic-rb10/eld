...
plugin::InputFile plugin::Section::getOldInputFile() const {
  if (!m_Section)
    return plugin::InputFile(nullptr);
  ELFSection *S = llvm::dyn_cast<ELFSection>(m_Section);
  if (!S)
    return plugin::InputFile(nullptr);
  CommonELFSection *commonSect = llvm::dyn_cast<CommonELFSection>(S);
  if (!commonSect)
    return plugin::InputFile(m_Section->getOldInputFile());
